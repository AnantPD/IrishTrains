...
class SearchTrainInteractor: PresenterToInteractorProtocol {
    ...
    func fetchallStations() {
        if Reach().isNetworkReachable() == true {
            APIManager.get(generalType: Stations.self,
                           url: URL(string: "http://api.irishrail.ie/realtime/realtime.asmx/getAllStationsXML")!)
            { result in
                switch(result) {
                case let .success(data):
                    self.presenter!.stationListFetched(list: data.stationsList)
    ...
    func fetchTrainsFromSource(sourceCode: String, destinationCode: String) {
        ...
        if Reach().isNetworkReachable() {
            APIManager.get(generalType: StationData.self,
                           url: URL(string: urlString)!)
            { result in
                switch(result) {
                case let .success(data):
                    self.proceesTrainListforDestinationCheck(trainsList: data.trainsList)
    ...
    private func proceesTrainListforDestinationCheck(trainsList: [StationTrain]) {
        ...
        for index  in 0...trainsList.count-1 {
            ...
            if Reach().isNetworkReachable() {
                APIManager.get(generalType: TrainMovementsData.self,
                               url: URL(string: _urlString)!) { result in
                    switch(result) {
                    case let .success(data):
                        let _movements = data.trainMovements
