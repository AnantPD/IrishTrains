...
class SearchTrainViewController: UIViewController {
    ...
    var selectedSourceStation: Station?
    ...
    @IBAction func favoriteStationTapped(_ sender: UIButton) {
        if let station = selectedSourceStation {
            let markFavorite = presenter?.favoriteStationTapped(stationId: station.stationId) ?? false
            sender.setImage(UIImage(systemName: markFavorite ? "heart.fill" : "heart")!, for: .normal)
        }
    }
...
extension SearchTrainViewController:PresenterToViewProtocol {
    ...
    func UpdateFavoriteButtonState() {
        ...
        DispatchQueue.main.async { [self] in
            if let station = selectedSourceStation {
                let favBtnImage = station.stationId != favStationId ? "heart" : "heart.fill"
                favoriteBtn.setImage(UIImage(systemName: favBtnImage)!, for: .normal)
            } else if favStationId != 0,
               let station = stationsList.first(where: { station in
                return station.stationId == favStationId
               }) {
                selectedSourceStation = station
                sourceTxtField.text = station.stationDesc
...
extension SearchTrainViewController:UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        dropDown.selectionAction = { (index: Int, item: String) in
            if textField == self.sourceTxtField {
                ...
                self.selectedSourceStation = self.stationsList[index]
                self.UpdateFavoriteButtonState()
            } else {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if let inputedText = textField.text {
            ...
            dropDown.dataSource = stationsList.map {$0.stationDesc}.filter {$0.contains(desiredSearchText)}
            dropDown.show()
